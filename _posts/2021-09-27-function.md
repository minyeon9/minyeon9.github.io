...
POSITION을 BOTH로 지정해 준 모습인데, 아무것도 지정하지 않았을 때와   
동일한 결과를 보여준다. POSITION 생략 시, BOTH의 역할을 한다고 생각하면 될 거 같다.   
...
```
SELECT ROUND(10.56789) FROM DUAL;      -- 10
```   
